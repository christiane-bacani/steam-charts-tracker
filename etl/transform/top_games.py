...
def transform_top_10_games(filepath: str) -> None:
    """
    Transform the extracted data of the current top 10 games (by current players) from
    a JSON file.
...
def transform_top_games_stats_overview(filepath: str) -> None:
    """
    Transform the extracted data of the current top 5 games' stats overview from a
    JSON file.
    ...
    try:
        # Parse the scraped data from a JSON file to perform data transformation
        with open(filepath, "r") as file:
            top_games_stats_overview = json.load(file)

        df = pd.DataFrame(top_games_stats_overview)
        ...
        # Store the DataFrame object to a CSV file of `data/output` directory
        df.to_csv("data/output/top_10_games_stats_overview.csv", index=False)
...
def transform_top_games_historical_stats(filepath: str) -> None:
    ...
    try:
        # Parse the scraped data from a JSON file to perform data transformation
        with open(filepath, "r") as file:
            top_games_historical_stats: dict[str, dict] = json.load(file)
        ...
        for app_id, historical_stats in top_games_historical_stats.items():
        ...
        provide_logs(
            "TRANSFORM",
            "Transform the extracted data of the top 10 games' historical "
            "stats from a JSON file.",
            "SUCCESSFUL",
            None
        )
        # Store the DataFrame object to a CSV file of `data/output` directory
        df.to_csv("data/output/top_10_games_historical_stats.csv", index=False)
        ...
        provide_logs(
            ...
            "FAILED",
            f"Filename: '{filepath}' is invalid for parsing the extracted data "
            "of the top 10 games' historical stats from a JSON file to perform "
            "data transformation."
        )
        raise FileNotFoundError("The given filename for parsing the extracted data "
                                "of the top 10 games' historical stats from a JSON "
                                "file to perform data transformation is invalid!")